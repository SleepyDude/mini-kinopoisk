...
export class InitService {
    ...
    async createAdminAndRoles(): Promise<boolean> {
        ...
        if (process.env.OWNER_MAIL === undefined || process.env.OWNER_PASSWORD === undefined) {
            throw new HttpRpcException('Ошибка инициализации, не найдены переменные среды OWNER_MAIL и OWNER_PASSWORD', HttpStatus.NOT_FOUND);
        }
