...
async function getRolesService(
  roleRepMock: any = rolesRepMocks.role({ id: 1, value: 1 }),
) {
...
describe('RolesService', () => {
  describe('createRole', () => {
    ...
    it('Error | userPerm = inf & releRep.create throw error', async () => {
      ...
      await expect(
        rolesService.createRole({ name: 'a', value: 17, description: 'b' }),
      ).rejects.toThrow(
        new RpcException('Ошибка при создании роли (роль уже существует)'),
      );
  ...
  describe('Delete Role by Name', () => {
    ...
    it('Error | userPerm = 17 & roleValue = 17 & role exists', async () => {
      ...
      await expect(rolesService.deleteByName('a', 17)).rejects.toThrow(
        new RpcException('Недостаточно прав'),
      );
      expect(rolesRep.findOne).toHaveBeenCalled();
    ...
    it('Error | userPerm = inf & role not exists', async () => {
      ...
      await expect(rolesService.deleteByName('a')).rejects.toThrow(
        new RpcException('Роли с таким именем не существует'),
      );
      expect(rolesRep.findOne).toHaveBeenCalled();
    });
  });
  ...
  describe('Update Role by Name', () => {
    it('Success | userPerm = inf & role exists', async () => {
      ...
      expect(await rolesService.updateByName('a', {}, Infinity)).toBe(
        roleModel,
      );
      expect(rolesRep.findOne).toHaveBeenCalled();
    ...
    it('Success | userPerm = 17 & roleValue = 10 -> 13', async () => {
      ...
      expect(
        await rolesService.updateByName('a', { value: roleNewValue }, userPerm),
      ).toBe(roleModel);
      expect(rolesRep.findOne).toHaveBeenCalled();
      expect(roleModel.update).toHaveBeenCalled();
    });
    ...
    it('Error | userPerm = 17 & roleValue = 10 -> 17', async () => {
      ...
      await expect(
        rolesService.updateByName('a', { value: roleNewValue }, userPerm),
    ...
    it('Error | userPerm = inf & role not found', async () => {
      ...
      await expect(
        rolesService.updateByName('a', { value: 5 }, Infinity),
